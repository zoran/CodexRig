...
import { initialFutureModulesDocument } from "../../../../scripts/docs/project-manifest-contract.mjs";
import { initialProjectManifest } from "../../../../scripts/docs/initial-project-manifest.mjs";
...
function writeIdentityDocs(targetRoot, projectName, projectDescription) {
  ...
  if (projectDescription)
    writeRelative(
      ...
      initialRequirementsDocument(projectDescription),
    );
  writeRelative(
    targetRoot,
    "AGENTS.md",
    markdown([
      ...
      "technical current-state inventory, and optional `docs/project-context.md` holds only bounded current work.",
      "",
      "## Start",
      "",
  ...
  writeRelative(
    ...
    markdown([
      "# " + displayName,
      "",
      "Start and use this project through the commands below; its documentation is linked separately.",
      "",
      ...generatedFrameworkReadmePolicy,
      ...(projectDescription ? ["", "- [Product requirements](docs/requirements.md)"] : []),
      "",
      "## Start",
      "",
      "Install [Codex CLI](https://developers.openai.com/codex/cli/), [mise](https://mise.jdx.dev/installing-mise.html), Git, Bash, ripgrep and ShellCheck. From this repository root run:",
      "",
      fence + "bash",
      supportedCodexStartCommand,
      fence,
      "",
      "Only `--no-alt-screen` and explicit Dev-only `--yolo` are launcher controls; enter prompts after native session selection. For a Dev session with full access, exit the current session and run `bash scripts/setup/start-codex.sh --yolo`.",
      "See [Session Start](instructions.md#session-start) for reconstruction, recovery and runtime permissions.",
      "",
      "For first setup:",
      "",
      fence + "bash",
      ...
      fence,
      "",
      "See [Dependency Installation And Freshness](instructions.md#dependency-installation-and-freshness) for compatible maintenance and failure handling.",
      "",
      "## Framework Updates",
      "",
      "Preview a reviewed newer framework checkout:",
      "",
      fence + "bash",
      "pnpm framework:upgrade -- --source <new-codexrig-root>",
      fence,
      "",
      "Review the plan before adding `--apply`. Complete the reported reconciliation and acknowledge its exact digest before verification. See [Framework Lifecycle](instructions.md#framework-lifecycle-compatibility-and-git-platforms) for the procedure and preservation rules.",
      "",
      "## Project Authority",
      "",
      "See [Project Instructions](instructions.md), the [safe-entry bootstrap](AGENTS.md) and [Codex configuration](.codex/README.md).",
      "",
  ...
  writeRelative(
    ...
    markdown([
      ...
      "The concise `docs/project.md` manifest indexes current technical facts and links the established",
      "requirements/design owners; it does not duplicate detailed product intent or future behavior.",
      "",
      ...generatedFrameworkInstructionsPolicy,
      ...requiredInstructionSection(sourceInstructions, "First-Prompt Project Definition Intake"),
      "",
      ...requiredInstructionSection(sourceInstructions, "Documentation Ownership"),
      "",
      ...
      "",
      "Update docs only when the user requested documentation, externally consumed usage/API/operations changed, or a durable project decision cannot be recovered from code, tests, configuration, or an existing canonical document. Use the established requirements/design owner, README for setup/use and manifest for current inventory; never create docs merely to record agent activity or prove a code change. Documentation has no general numeric line or word quota;",
      "only the always-loaded root `AGENTS.md` has a 24 KiB bootstrap cap within the configured 32 KiB project-instruction budget. This file owns complete workflow policy, the README owns setup/use, the manifest owns current reality, Future Modules owns confirmed deferred candidates, and each focused document needs a distinct audience and maintenance owner. Bootstrap safety projections summarize and link; README links documentation without summarizing its contents; skills expose selection metadata and load their full `SKILL.md` only when relevant. Larger context windows never authorize duplicated or stale policy, and the byte budget never authorizes hiding framework elements.",
  ...
  writeRelative(
    ...
    initialProjectManifest({
      displayName,
      hasCreationBrief: Boolean(projectDescription),
...
function main() {
  ...
  try {
    ...
    assertGeneratedProjectClean(stagingRoot, packageName, { projectDescription });
    assertGeneratedProjectParity({
