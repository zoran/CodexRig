...
import { generatedFrameworkInstructionsPolicy } from "./generated-framework-policy.mjs";
import {
  generatedFrameworkAgentPolicy,
  generatedFrameworkReadmePolicy,
} from "./generated-framework-projections.mjs";
import {
...
function writeIdentityDocs(targetRoot, projectName, projectDescription) {
  ...
  writeRelative(
    ...
    markdown([
      ...
      "Codex, pnpm, and local hook-shell executables to absolute paths outside the writable root, atomically",
      "reserves the selected launcher lease, and rejects executable or unknown ignored runtime",
      "configuration. It then injects two session-owned lifecycle hooks and requires Codex's stable",
      "`hooks/list` result to return a warning-free inventory containing",
      "exactly those two definitions as enabled and trusted without a global trust bypass. Tracked",
      "`.codex/hooks.json` has no executable handler, and any additional hook blocks canonical startup.",
      ...
      "reconstruction gate; the primary begins with the full Git/worktree/session inventory, where one",
      "stale sibling cannot suppress safe reconstruction of the rest.",
      "",
      ...
      "",
      "After every completed slice, run the lightweight read-only Worktree Settlement trigger with `pnpm worktree:status -- --json` and recheck session, lease, recovery, task-branch, handover, transaction, reservation, and preservation-lock claims. Preservation is a safety state, never completion: integrate or explicitly retire every no-longer-needed item, or retain a concrete blocker and resolution condition. A completed goal stays open until its no-longer-needed goal-owned temporary worktrees and related coordination residue are settled; the current owning session remains only until mandatory post-exit reset.",
      "After the mutating work of every completed goal and before its goal-wide documentation review/final audit, run `pnpm repo:housekeeping -- --apply`. It accepts Git-less roots and clears proven-dead leases, repairs missing or invalid recovery from the exact active-phase lease during normal release or stale cleanup, and retains any valid latest marker; a current process identity observed outside its bound PID namespace or otherwise mechanically indeterminate remains an ownership-confirmation blocker. It preserves any existing directory with a broken Git worktree link instead of repairing it automatically. It prunes only missing registrations whose paths remain held by exact process-bound non-directory reservations across the native prune while process-bound Git locks protect every non-missing linked sibling. Before native prune, one shared process-bound transaction in the Git common directory records every reserved path and preservation reason, so post-prune crashes remain discoverable. A failed or changed reservation leaves its registration intact. After an interrupted cleanup, only unchanged current-contract transactions, reservations, or preservation locks whose exact owner is proven dead are retired automatically. Missing or unobservable Linux procfs is indeterminate. Only after the primary or developer confirms the directory still belongs to that registration may the primary run explicit native Git repair and resume housekeeping. It preserves unsafe or ambiguous state; an invalid orphan recovery file remains a visible advisory rather than invented writer ownership. It reconciles unambiguous delivery evidence and the bounded manifest projection, formats the result, and checks repository architecture/docs/headers, localization, Identity and Access, white-label configuration, dependencies, secrets, agent policy, and framework health. It never removes a worktree directory, deploys, commits, pushes, or changes external state. The weekly GitHub/GitLab schedules run the read-only online mode; ambiguous staging/production evidence requires developer classification.",
      "",
      "## Project Authority",
      "",
