...
function writeIdentityDocs(targetRoot, projectName) {
  ...
  writeRelative(
    ...
    markdown([
      ...
      "sources once per durable local turn. It also validates an optional bounded `docs/project-context.md`",
      "`codexrig-work-state` marker and reopens active work. Only a durable local Stop event with a",
      ...
      "is resume metadata rather than authority.",
  ...
  writeRelative(
    ...
    markdown([
      ...
      "already-continued durable turn is allowed to stop instead of looping; semantic search repairs",
  ...
  writeRelative(
    ...
    markdown([
      ...
      "  Stop hook refreshes changed sources and validates the bounded `codexrig-work-state` marker only",
      "  for durable local Stop events with a non-null `transcript_path`; semantic search retains on-demand",
      "  repair. Ephemeral side conversations and other transcriptless contexts exit before work-state or",
      "  index access and cannot reopen parent-thread work. An unchanged already-continued durable turn is allowed",
      "  to stop instead of looping; a changed revision can continue again. New or changed hook hashes",
      "  require local `/hooks` approval.",
