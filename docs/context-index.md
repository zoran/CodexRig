...
Every framework reset removes the complete ignored setup-created index, including its model cache,
through the same ownership and maintenance-lock checks as `context:clean`. The next `pnpm setup`,
`pnpm context:index`, or semantic search rebuilds it and may need to download the pinned model
again. Generated projects and portable exports still exclude all vector and model state and
bootstrap their own index.
...
interrupted model-hash temporary files, and the identical transient `config.json` that the current
model download writes beside the selected revision. Candidate trees are bounded and must have the
expected generated name, type, link count, ownership, and stable device/inode identity. Unknown,
malformed, ambiguous, symlinked, hardlinked, or identity-changing state fails closed and remains for
explicit inspection. Routine maintenance validates and classifies state without mutating the
selected LanceDB table.
...
until the new pair commits atomically, so schema-mismatched state rebuilds automatically without
deletion or manual Lance commands. Crash recovery keeps the last complete selected pair recoverable.
Normal output remains a path-free sanitized count summary.

`context:clean` is the explicit standalone operation that removes the complete project-owned index
after acquiring the same lock. `framework:reset --apply` invokes that removal boundary as a
mandatory reset step. Both are intentionally broader than opportunistic maintenance and still
require the ownership marker and safe fixed project path. There is currently no separate production
