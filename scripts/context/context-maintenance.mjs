...
function emptySummary() {
  return {
    ...
    removedTransientModelArtifacts: 0,
...
function totalRemoved(summary) {
  return (
    ...
    summary.removedTransientModelArtifacts +
...
function modelMaintenanceInventory({ modelCachePath, selectedModelRevisionDirectory }) {
  ...
  const transientConfigPath = path.join(modelParent, "config.json");
  ...
  while (pending.length > 0) {
    ...
    for (const entry of readdirSync(current.directory, { withFileTypes: true })) {
      ...
      } else if (entryPath === transientConfigPath) {
        transientConfig = readStableArtifactFile(entryPath, "transient model-download config");
        if (transientConfig.stats.dev !== modelCacheDevice) {
  ...
  if (transientConfig) {
    if (!existsSync(selectedConfigPath)) {
      throw new Error("Context maintenance refused an unpaired transient model config.");
    }
    const selectedConfig = readStableArtifactFile(selectedConfigPath, "selected model config");
    if (selectedConfig.stats.dev !== modelCacheDevice) {
      throw new Error("Context maintenance refused foreign-filesystem model config.");
    }
    if (!transientConfig.buffer.equals(selectedConfig.buffer)) {
      throw new Error("Context maintenance refused a divergent transient model config.");
    }
  }
  return {
    claims,
    ...
    transientConfigPath,
    transientConfigRemovable: Boolean(transientConfig),
  };
}
...
function removeUnselectedModelRevisions({
  ...
  if (inventory.transientConfigRemovable) {
    const transient = readStableArtifactFile(
      inventory.transientConfigPath,
      "transient model-download config",
    );
    const selected = readStableArtifactFile(inventory.selectedConfigPath, "selected model config");
    if (
      !transient.buffer.equals(selected.buffer) ||
      !sameStableIdentity(transient.stats, lstatSync(inventory.transientConfigPath)) ||
    ...
    summary.removedTransientModelArtifacts += 1;
  }
}
    ...
    pending:
      ...
      Boolean(
        ...
        (modelInventory.claims.length > 0 ||
          ...
          modelInventory.transientConfigRemovable ||
