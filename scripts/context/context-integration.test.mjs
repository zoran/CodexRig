...
test(
  ...
  async (context) => {
    ...
    const repairStatus = spawnSync(process.execPath, [checkScript], {
      cwd: repositoryRoot,
      env,
      encoding: "utf8",
      timeout: 30_000,
    ...
    const schemaMismatchManifest = {
      ...repairManifest,
      schemaVersion: schemaVersion - 1,
    };
    writeFileSync(manifestPath, `${JSON.stringify(schemaMismatchManifest, null, 2)}\n`, "utf8");
    const databaseInodeBeforeSchemaRepair = statSync(databasePath).ino;
    ...
    const schemaRepairedManifest = JSON.parse(readFileSync(manifestPath, "utf8"));
    assert.equal(validateManifest(schemaRepairedManifest).valid, true);
    assert.equal(schemaRepairedManifest.schemaVersion, schemaVersion);
    assert.equal(schemaRepairedManifest.stats.reusedChunks, 0);
    assert.equal(schemaRepairedManifest.stats.databaseIndexComplete, true);
    assert.notEqual(statSync(databasePath).ino, databaseInodeBeforeSchemaRepair);
    context.diagnostic(
      JSON.stringify({
        firstWallMs,
