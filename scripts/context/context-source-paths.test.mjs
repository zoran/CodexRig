...
test("normal context commands cannot be redirected to another project or model cache", () => {
  ...
  const redirected = spawnSync(process.execPath, [script], {
    cwd: repositoryRoot,
  ...
  const redirectedDirectory = spawnSync(process.execPath, [script], {
    cwd: repositoryRoot,
...
test("context check is read-only on a missing index and rejects unused options", () => {
  ...
  const unsupported = spawnSync(process.execPath, [script, "--status-only"], {
    cwd: repositoryRoot,
    env: environment,
    encoding: "utf8",
    timeout: 2_000,
  ...
  assert.match(`${unsupported.stdout}${unsupported.stderr}`, /Unknown context check argument/);
