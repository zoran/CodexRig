...
function normalizeSection(value) {
  ...
  return dependencySections.includes(text) ? text : null;
}

function isCompleteOutdatedEntry(entry) {
  return (
    isRecord(entry) &&
    nonemptyScalar(entry.packageName) &&
    nonemptyScalar(entry.current) &&
    nonemptyScalar(entry.wanted) &&
    nonemptyScalar(entry.latest) &&
    normalizeSection(entry.dependencyType) !== null
  );
}
...
function singularOwner(entry, manifests, projectRoot, packageName, section) {
  const names = [entry.dependentPackageName].filter(
    (value) => value !== undefined && value !== null,
  );
  const locations = [entry.dependentPackageLocation].filter(
    (value) => value !== undefined && value !== null,
  );
...
export function normalizeOutdated(
  ...
  for (const entry of rawOutdatedEntries(raw)) {
    ...
    const registryName = entry.packageName;
    const section = normalizeSection(entry.dependencyType);
  ...
  for (const manifest of manifests) {
    for (const section of ["dependencies", "devDependencies", "optionalDependencies"]) {
      for (const name of Object.keys(manifest.data[section] ?? {}).sort()) {
        ...
        if (normalizeSection(entry.dependencyType) !== section) {
...
function recommendation(entry, policy) {
  ...
  if (delta === "minor") return "Select after integration review.";
  if (delta === "major") return "Select after breaking-change review.";
...
export function latestPatchTarget(entry) {
  ...
  if (typeof entry.registryName !== "string" || !entry.registryName.trim()) {
    throw new Error("Dependency entry is missing its canonical registry name.");
  }
  return (
    registryVersions(entry.registryName)
      .map(parseVersion)
      .filter(Boolean)
      .filter(
        (version) =>
