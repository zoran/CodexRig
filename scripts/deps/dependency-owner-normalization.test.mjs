...
test("pins and exact selectors stay scoped to manifest and dependency section", () => {
    ...
    pins: [
      {
        ...
        reason: "Domain major upgrade is separate",
      },
    ],
  };
  ...
  for (const shorthand of [
    ...
    "packages/domain/package.json:shared",
  ]) {
    assert.throws(
      () => selectDependencyEntries(crossSection, [shorthand]),
      /must use <manifest>:<section>:<package>/i,
    );
  }
});
...
test("noncanonical pnpm field aliases fail closed", () => {
  const fixture = workspaceFixture();
  const versions = {
    current: "6.0.3",
    wanted: "6.0.4",
  ...
  assert.throws(
    () =>
      normalize([{ ...versions, name: "typescript", dependencyType: "devDependencies" }], fixture),
    /incomplete package record/,
  );
  assert.throws(
    () => normalize({ typescript: { ...versions, type: "devDependencies" } }, fixture),
    /incomplete package record/,
  );
  assert.throws(
    () => normalize({ typescript: { ...versions, dependencyType: "devDependency" } }, fixture),
    /incomplete package record/,
  );
  for (const ownerAlias of [
    { dependent: "@example/domain" },
    { dependentLocation: path.join(fixture.root, "packages/domain") },
  ]) {
    assert.throws(
      () =>
        normalize(
      ...
      /did not identify one manifest dependency/,
    );
  }
  assert.throws(
    () => latestPatchTarget({ current: "6.0.3", name: "typescript" }),
