#!/usr/bin/env node
/** Maintains the startup toolchain, compatible dependencies, and CI pins through framework-owned transactions. */
import {
  chmodSync,
  mkdirSync,
  mkdtempSync,
...
import { ciAdapterContractViolations, frameworkInstallationFindings } from "./framework-doctor.mjs";
...
export async function maintainToolchain({
  ...
  try {
    ...
    const installation = frameworkInstallationFindings({ root });
    if (installation.errors.length)
      throw new Error(installation.errors.map((entry) => entry.message).join("; "));
    const inputs = toolchainMaintenanceInputs(root);
    const current = readCompatibilityMatrix(root);
    ...
    stageWrite(stage, ".codexrig/framework.json", inputs.contents[".codexrig/framework.json"]);
    ...
    run(
      ...
      [
        ...
        "--",
        "node",
        path.join(frameworkRoot, "scripts/deps/install-compatible.mjs"),
        "--stage-toolchain",
      ],
      stageOptions,
    );
    ...
    const receipt = receiptWithMaintainedPackageManager(inputs.receipt, packageJson.packageManager);
    if (receipt) desired[inputs.contract.upgrade.receiptFile] = receipt;
    verifyInputRecords(root, allInputs);
...
async function main() {
  ...
  if (args.some((arg) => arg !== "--startup") || args.length > 1)
    throw new Error("Usage: node scripts/framework/maintain-toolchain.mjs [--startup]");
...
if (process.argv[1] === fileURLToPath(import.meta.url))
  main().catch((error) => {
    console.error(`Startup maintenance failed: ${formatContextError(error, frameworkRoot)}`);
    process.exitCode = 1;
  });
