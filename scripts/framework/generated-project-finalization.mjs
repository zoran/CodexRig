...
import { listManagedMarkdownFiles } from "../docs/document-scope.mjs";
...
import { readStableRepositoryText } from "../repository/stable-file-snapshot.mjs";
...
function assertDeclaredConfigurationTransformations({ sourceRoot, targetRoot, transferManifest }) {
  ...
  if (memoryFlagCount !== 1 || targetConfig !== projectOutputText(expectedConfig)) {
  ...
  const expectedPackage = selectedProjectPackage(sourcePackage, selection);
  ...
  if (
    readFileSync(path.join(targetRoot, ".codex/tooling.json"), "utf8") !==
    projectOutputText(
    ...
    fail("Generated tooling configuration differs from the project contract.");
  const sourceVerification = JSON.parse(
    readFileSync(path.join(sourceRoot, ".codex/verification.json"), "utf8"),
  );
  if (
    JSON.stringify(
      JSON.parse(readFileSync(path.join(targetRoot, ".codex/verification.json"), "utf8")),
    ) !==
    projectOutputText(JSON.stringify(selectedProjectVerification(sourceVerification, selection)))
  )
    fail("Generated verification differs from its selected project capabilities.");
  for (const file of [".github/workflows/ci.yml", ".gitlab-ci.yml"])
    if (
      readFileSync(path.join(targetRoot, file), "utf8") !==
      projectOutputText(
        projectCiProjection(file, readFileSync(path.join(sourceRoot, file), "utf8")),
      )
    )
      fail("Generated CI differs from its selected stable adapter.");
}
  ...
  const changedPaths = transferManifest.files
    ...
    .filter(
      ({ identity, relativePath }) =>
        ...
        ) !== readStableRepositoryText({ repositoryRoot: targetRoot, relativePath }).text,
    )
...
export function updateGeneratedPackage(sourceRoot, targetRoot, packageName) {
  const packagePath = path.join(targetRoot, "package.json");
  const packageJson = selectedProjectPackage(
...
export function assertGeneratedProjectClean(
  ...
  for (const forbidden of [
    ...
    "scripts/framework/source-baseline.mjs",
  ]) {
