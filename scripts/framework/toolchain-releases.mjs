...
import { validateCompatibilityMatrix } from "../contracts/framework-contract.mjs";
...
export async function resolveToolchainReleases(current, { fetchImpl = globalThis.fetch } = {}) {
  const matrix = structuredClone(validateCompatibilityMatrix(current));
  ...
  for (const canary of matrix.canaries) {
    if (canary.node === current.stable.node.version) canary.node = matrix.stable.node.version;
    if (canary.pnpm === current.stable.pnpm.version) canary.pnpm = matrix.stable.pnpm.version;
  }
  if (JSON.stringify(matrix) !== JSON.stringify(current))
    matrix.reviewedOn = new Date().toISOString().slice(0, 10);
  return validateCompatibilityMatrix(matrix);
}
