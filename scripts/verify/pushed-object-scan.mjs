...
function gitInvocation(repositoryRoot, args) {
  ...
  const metadata = gitMetadataByRoot.get(repositoryRoot);
  if (!metadata) throw new Error("Pushed-object scan requires project-owned Git metadata.");
  return {
    args: isolatedGitArguments({
      args,
...
function assertNoReplacementRefs(repositoryRoot) {
  const replacementRefs = git(repositoryRoot, [
    "for-each-ref",
    "--format=%(refname)",
    "refs/replace",
  ])
  ...
  if (replacementRefs.length > 0) {
    throw new Error(
      `Pushed-object scan refuses active Git replacement refs: ${replacementRefs.join(", ")}`,
    );
  }
}
...
async function scanGitObjects(repositoryRoot, objects) {
  ...
  const invocation = gitInvocation(repositoryRoot, ["cat-file", "--batch"]);
...
export async function inspectPushedObjects(input, { repositoryRoot = root } = {}) {
  assertNoReplacementRefs(repositoryRoot);
