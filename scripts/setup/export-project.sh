...
archive_root="$(
  node --input-type=module --eval '
    ...
    if (
      ...
      throw new Error("package.json name must be a safe npm package name before export.");
    }
    process.stdout.write(name.replace(/^@/u, "").replace("/", "-"));
  '
)"
...
output="${1:-dist/exports/$archive_root.tar.gz}"
