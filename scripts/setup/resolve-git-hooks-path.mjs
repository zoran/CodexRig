...
export function resolveGitHooksPath({
  ...
  commonDirectory: commonDirectoryArgument,
  hooksDirectory: hooksDirectoryArgument,
} = {}) {
  if (!repositoryRootArgument || !commonDirectoryArgument || !hooksDirectoryArgument) {
    fail("Git hook resolution requires repository, common-directory, and hooks-directory paths.");
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  try {
    const [repositoryRoot, commonDirectory, hooksDirectory] = process.argv.slice(2);
    console.log(resolveGitHooksPath({ commonDirectory, hooksDirectory, repositoryRoot }));
  } catch (error) {
